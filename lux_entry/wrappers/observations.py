...
class ObservationWrapper(gym.ObservationWrapper):
    def __init__(self, env: gym.Env) -> None:
        ...
        n_players = 2
        x, y = self.env.state.env_cfg.map_size // 4, self.env.state.env_cfg.map_size // 4
    ...
    def observation(self, observation: ObservationStateDict) -> Dict[str, np.ndarray]:
        ...
        obs = dict()
        for key, value in self.observation_space.keys():
            if value.shape[2] > 1:
                obs[key] = (
                    np.zeros(value.shape)
                    if value.dtype == spaces.MultiBinary
                    else np.zeros(value.shape) + value.low
                )
        ...
        for p, player in enumerate(["player_0", "player_1"]):
            for factory in observation.factories[player].values():
                ...
                obs["normalized_factory_ice"][p][factory.pos] = factory.ice / reasonable_factory_cargo_space
                obs["normalized_factory_ore"][p][factory.pos] = factory.ore / reasonable_factory_cargo_space
                obs["normalized_factory_water"][p][factory.pos] = factory.water / (reasonable_factory_cargo_space / ice_water_ratio)
                obs["normalized_factory_metal"][p][factory.pos] = factory.metal / (reasonable_factory_cargo_space / ore_metal_ratio)
                obs["normalized_factory_power"][p][factory.pos] = factory.power / reasonable_factory_power
                obs["total_normalized_factory_ice"][p] += factory.ice / (reasonable_factory_cargo_space * max_factories)
                obs["total_normalized_factory_ore"][p] += factory.ore / (reasonable_factory_cargo_space * max_factories)
                obs["total_normalized_factory_water"][p] += factory.water / (reasonable_factory_cargo_space * max_factories / ice_water_ratio)
                obs["total_normalized_factory_metal"][p] += factory.metal / (reasonable_factory_cargo_space * max_factories / ore_metal_ratio)
            ...
            for robot in observation.units[player].values():
                ...
                obs["normalized_light_robot_ice"][p][robot.pos] = robot.ice / light_cargo_space
                obs["normalized_light_robot_ore"][p][robot.pos] = robot.ore / light_cargo_space
                obs["normalized_heavy_robot_power"][p][robot.pos] = robot.power / heavy_battery_cap
                obs["normalized_heavy_robot_ice"][p][robot.pos] = robot.ice / heavy_cargo_space
                obs["normalized_heavy_robot_ore"][p][robot.pos] = robot.ore / heavy_cargo_space
            ...
            obs["has_lichen"][p] = observation.board.lichen_strains in lichen_strains[p]
            obs["normalized_lichen"][p] = observation.board.lichen / max_lichen * (observation.board.lichen_strains in lichen_strains[p])
            obs["total_normalized_lichen"][p] = sum(obs["normalized_lichen"][p])
